...
def for_testing():
    ...
    file = dir.parent / "chapters" / "html" / "001-Chapter-1-Prologue.html"
    with open(file) as f:
...
def get_title(soup):
    # title = soup.find("h1", class_="wp-block-post-title") #this doesn't work with offile files
    # del title["class"]
    div = soup.find(
        "div",
...
def get_paragraphs(soup):
    ...
    # paragraphs = block.find_all(["p", "hr", "li"])
    for p in paragraphs:
